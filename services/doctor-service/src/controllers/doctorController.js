...
export const createDoctorProfile = async (req, res) => {
  try {
    ...
    const { name, email, phone, specialization, licenseNumber, hospital, qualifications, experience, consultationFee, bio } = req.body;
...
export const updateDoctorProfile = async (req, res) => {
  try {
    const allowed = ['name','phone','specialization','hospital','qualifications','experience','consultationFee','bio','profileImage'];
    const updates = {};
    allowed.forEach(f => { if (req.body[f] !== undefined) updates[f] = req.body[f]; });
    const doctor = await Doctor.findOneAndUpdate(
      { authUserId: req.user.userId }, updates, { new: true, runValidators: true }
    );
    if (!doctor) return res.status(404).json({ success: false, message: 'Profile not found' });
...
export const setAvailability = async (req, res) => {
  try {
    ...
    const doctor = await Doctor.findOneAndUpdate(
      { authUserId: req.user.userId }, { availability }, { new: true }
    );
    if (!doctor) return res.status(404).json({ success: false, message: 'Profile not found' });
    res.status(200).json({ success: true, message: 'Availability updated', data: { availability: doctor.availability } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const getAllApprovedDoctors = async (req, res) => {
  try {
    ...
    const doctors = await Doctor.find(filter)
      .select('name specialization hospital experience consultationFee bio rating availability');
    res.status(200).json({ success: true, message: 'Doctors fetched', data: { count: doctors.length, doctors } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
