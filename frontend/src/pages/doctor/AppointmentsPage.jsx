...
import { getDoctorAppointments, confirmAppointment, cancelAppointment, completeAppointment } from "../../services/appointmentApi";
...
function DoctorAppointmentsPage() {
  ...
  const handleConfirm = async id => {
    ...
    try { await confirmAppointment(id); msg("ok", "Appointment confirmed."); load(); }
    catch (err) { msg("err", err.response?.data?.message || "Failed to confirm."); }
  ...
  const handleDecline = async id => {
    ...
    try { await cancelAppointment(id, "Declined by doctor"); msg("ok", "Appointment declined."); load(); }
    catch (err) { msg("err", err.response?.data?.message || "Failed to decline."); }
    finally { setActionId(null); }
  };

  const handleComplete = async id => {
    setActionId(id + "complete");
    try { await completeAppointment(id, "Consultation completed."); msg("ok", "Marked as completed."); load(); }
    catch (err) { msg("err", err.response?.data?.message || "Failed."); }
    finally { setActionId(null); }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Review and manage patient appointment requests</p>
      </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1,2,3].map(i => <div key={i} className="animate-pulse h-36 rounded-2xl border bg-white" />)}
        </div>
      ) : filtered.length === 0 ? (
          ...
          {filtered.map(appt => (
            <div key={appt._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="flex flex-wrap gap-4 text-xs text-slate-500 mb-3">
                <span>📅 {new Date(appt.appointmentDate).toLocaleDateString("en-LK", { year: "numeric", month: "short", day: "numeric" })}</span>
                <span>🕐 {appt.timeSlot}</span>
                <span className="capitalize">📱 {appt.type}</span>
                {appt.consultationFee > 0 && <span>💰 LKR {appt.consultationFee.toLocaleString()}</span>}
              </div>

              <div className="rounded-xl bg-slate-50 px-3 py-2 mb-3">
              ...
              <div className="flex gap-2 flex-wrap">
                {appt.status === "pending" && (
                  <>
                    <button onClick={() => handleConfirm(appt._id)} disabled={!!actionId}
                      className="flex items-center gap-1 rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-500 disabled:opacity-60">
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      {actionId === appt._id + "confirm" ? "Confirming..." : "Accept"}
                    </button>
                ...
                {appt.status === "confirmed" && (
                  <>
                    ...
                    <button onClick={() => handleComplete(appt._id)} disabled={!!actionId}
                      className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-600 hover:bg-blue-100 disabled:opacity-60">
