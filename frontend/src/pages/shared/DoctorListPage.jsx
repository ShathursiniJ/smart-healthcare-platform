  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Browse verified doctors and book an appointment</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-slate-400 mt-1">Try a different search or specialty</p>
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-4">
          {filtered.map((doctor, index) => (
            <div key={doctor._id}
              ...
              <div className="flex items-start gap-3 mb-4">
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-sm font-bold text-white ${AVATAR_COLORS[index % AVATAR_COLORS.length]}`}>
                  {doctor.profileImage
                    ? <img src={doctor.profileImage} alt="" className="h-full w-full rounded-xl object-cover" />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {doctor.hospital && (
                    <p className="text-xs text-slate-400 truncate mt-0.5">{doctor.hospital}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-xs text-slate-500 mb-4">
                <span className="flex items-center gap-1">
                  ...
                  {doctor.experience ? `${doctor.experience} yrs exp` : 'Experienced'}
                </span>
                <span className="flex items-center gap-1">
              ...
              <button
                onClick={e => { e.stopPropagation(); navigate(`/patient/find-doctors/${doctor._id}`); }}
                className="w-full rounded-xl bg-teal-600 py-2 text-sm font-semibold text-white hover:bg-teal-500 transition">
                Book Appointment
              </button>
            </div>
          ))}
