    ...
    <div className="space-y-6">
      ...
      {preSelected && (
        <div className="rounded-2xl border border-teal-200 bg-teal-50 p-5 shadow-sm">
          <p className="text-xs font-semibold text-teal-600 uppercase tracking-wide mb-3">Ready to Join</p>
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold text-slate-800">{preSelected.doctorName}</p>
              <p className="text-sm text-slate-500">{preSelected.specialization}</p>
            ...
            <button onClick={() => joinSession(preSelected)} disabled={joining === preSelected._id}
              className="flex items-center gap-2 rounded-xl bg-teal-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-teal-500 transition disabled:opacity-60">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-500">No confirmed video appointments</p>
            ...
            {appointments.map(appt => (
              <div key={appt._id}
                ...
                <button onClick={() => joinSession(appt)} disabled={joining === appt._id}
                  className="flex items-center gap-1 rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-500 transition disabled:opacity-60">
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                  </svg>
