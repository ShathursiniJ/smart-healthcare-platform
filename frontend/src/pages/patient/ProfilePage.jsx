...
function ProfilePage() {
  ...
  const handleCancel = () => {
    ...
    setSuccessMessage("");
    setErrorMessage("");
    // Only exit edit mode if profile exists
    if (profileExists) {
      setIsEditing(false);
    } else {
      setErrorMessage("Create your profile to get started");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Validate that if any emergency field is filled, all must be filled
      if (hasAnyEmergencyField) {
        if (
          ...
          setErrorMessage(
            "Please fill in all emergency contact details (name, relationship, and phone)."
          );
          setSubmitting(false);
          return;
        }
