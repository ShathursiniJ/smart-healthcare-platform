  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">View and manage platform payment transactions</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="grid grid-cols-6 border-b border-slate-100 bg-slate-50 px-4 py-3 text-xs font-medium text-slate-500">
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map(payment => (
              <div key={payment._id} className="grid grid-cols-6 items-center px-4 py-3 hover:bg-slate-50 transition">
                <span className="text-xs font-mono text-slate-500 truncate">
                  {payment.transactionId || payment._id?.slice(-8) || '—'}
                </span>
                <span className="text-sm font-medium text-slate-800 truncate">{payment.patientName}</span>
                <span className="text-sm text-slate-600 truncate">{payment.doctorName}</span>
