...
import { getAllDoctors, deactivateDoctor, activateDoctor } from "../../services/doctorApi";
...
function ManageDoctorsPage() {
  ...
  const activeDoctors = doctors.filter((d) => d.approvalStatus === "approved" && d.isActive);
  ...
  const handleActivate = async (id) => {
    ...
    try {
      ...
      setSuccessMessage("Doctor activated");
      fetchDoctors();
      setTimeout(() => setSuccessMessage(""), 3000);
    } catch {
      // handle error
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Active", value: activeDoctors.length, color: "text-slate-800" },
      ...
      <div className="grid grid-cols-3 gap-4">
        {filtered.length === 0 && (
          <div className="col-span-3 rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500 shadow-sm">
            ...
            <div className="mt-4 flex gap-2">
              ...
              {!doctor.isActive && doctor.approvalStatus === "approved" && (
                <button
                  ...
                  disabled={actionId === doctor._id}
                  className="flex h-9 w-9 items-center justify-center rounded-xl border border-emerald-100 hover:bg-emerald-50 disabled:opacity-50"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-800">{selectedDoctor.name}</h2>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <p className="text-slate-700">{selectedDoctor.email}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Phone</p>
                <p className="text-slate-700">{selectedDoctor.phone || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">License Number</p>
                <p className="text-slate-700">{selectedDoctor.licenseNumber || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Experience</p>
                <p className="text-slate-700">{selectedDoctor.experience || 0} years</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Consultation Fee</p>
                <p className="text-slate-700">LKR {(selectedDoctor.consultationFee || 0).toLocaleString()}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500">Approval Status</p>
                <p className="capitalize text-slate-700">{selectedDoctor.approvalStatus || "pending"}</p>
              </div>
            </div>
            ...
            {Array.isArray(selectedDoctor.qualifications) && selectedDoctor.qualifications.length > 0 && (
              <div className="mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-700">
                <p className="mb-1 text-xs text-slate-500">Qualifications</p>
